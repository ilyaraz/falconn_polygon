...
double evaluate(PlainArrayPointSet<float> points,
                const VectorXf &center,
                PlainArrayPointSet<float> queries,
                int32_t num_tables,
                int32_t num_bits,
                const vector<vector<uint32_t>> &knn,
                ofstream *log) {
    ...
    double table_size = double((uint64_t(1) << num_bits) + points.num_points) * s / 8.0;
    ...
    for (int32_t num_threads = 1; num_threads <= 4; ++num_threads) {
        ...
        double query_time = duration_cast<duration<double>>(t2 - t1).count() / queries.num_points;
